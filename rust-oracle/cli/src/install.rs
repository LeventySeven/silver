...
pub fn find_installed_chrome() -> Option<PathBuf> {
    ...
    let debug = std::env::var("SILVER_DEBUG").is_ok();
...
pub fn run_install(with_deps: bool) {
    if cfg!(all(target_os = "linux", target_arch = "aarch64")) {
        ...
        eprintln!("  Then use: silver --executable-path /usr/bin/chromium");
        exit(1);
    }
    ...
    if is_linux {
        if with_deps {
            ...
            println!("  silver install --with-deps");
            println!();
        }
    }
...
mod tests {
    ...
    async fn http_client_sends_user_agent() {
        ...
        let expected_ua = format!("silver/{}", env!("CARGO_PKG_VERSION"));
        assert!(
