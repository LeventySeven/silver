...
const NAME = `moxxie-harden-${process.pid}-${Date.now()}`
...
describe('security hardening (real Chromium via the run() entry)', () => {
  ...
  it('screenshot refuses an absolute / traversal path outside CWD', async () => {
    ...
    const abs = await run(['screenshot', '/etc/moxxie-evil.png', '--session', NAME])
    expect(abs.env.success).toBe(false)
    expect(abs.env.error).toBe(ERRORS.path_denied.message)

    const trav = await run(['screenshot', '../../moxxie-evil.png', '--session', NAME])
  ...
  it('skill serves on-disk SKILL.md when present, else an inline fallback', async () => {
    ...
    if (!preExisted) {
      ...
      expect(String(fb.env.data).toLowerCase()).toContain('moxxie')
    }

    let created = false
    if (!preExisted) {
      mkdirSync(dir, { recursive: true })
      writeFileSync(file, '# moxxie SKILL\nSENTINEL_ONDISK_DOC\n', 'utf8')
      created = true
    }
    try {
